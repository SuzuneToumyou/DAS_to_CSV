...
tkinter.messagebox.showinfo('DASデータをCSV変換','入力ファイルを選択してください！')
input_file = tkinter.filedialog.askopenfilename(filetypes = fTyp,initialdir = iDir)
# 処理ファイル名の出力
tkinter.messagebox.showinfo('CSV変換入力ファイル',input_file)

# 出力フォルダ選択ダイアログの表示fld = filedialog.askdirectory(initialdir = dir) 
tkinter.messagebox.showinfo('DASデータをCSV変換','出力フォルダを選択してください！')
o_name=filedialog.askdirectory(initialdir = iDir) 
tkinter.messagebox.showinfo('CSV変換出力フォルダ',o_name)
